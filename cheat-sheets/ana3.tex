...
\begin{defn}
  ...
  \[ \tilde{\Alg}_\mu \coloneqq \Set{ A \cup N }{ A \in \Alg, N \in \mathfrak{N}_\mu }. \]
  Dann gilt:
  \begin{itemize}
    \item $\tilde{\Alg}_\mu = \Alg(\mathfrak{N}_\mu \cup \Alg)$, ist also eine $\sigma$-Algebra.
    \item Die Funktion $\mu : \tilde{\Alg}_\mu \to [0, \infty]$ definiert durch $\tilde\mu(\tilde{A}) \coloneqq \mu(A)$, wenn $\tilde{\Alg}_\mu \ni A \cup N$ mit $A \in \Alg$ und $N \in \mathfrak{N}$, ist ein Maß.
  \end{itemize}
\end{defn}
...
\begin{defn}
  Sei $f \in \E(\Omega, \Alg)$ und $\textstyle f = \sum_{j=1}^k y_j \mathbbm{1}_{A_j}$ eine kanonische Darstellung von $f$. Sei ferner $\mu$ ein Maß auf $\Alg$. Dann heißt die Größe
