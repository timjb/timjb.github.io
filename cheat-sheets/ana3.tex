...
% Kleinere Vereinigungs-, Schnitt- und Summensymbole
\let\myBigcup\bigcup
...
\begin{satz}
  \begin{doublespace}
    Sei $(f_n)_{n \in \N}$ Folge messbarer Fkt. $f_n : (\Omega, \Alg, \mu) \to (\ER, \overline{\Bor})$ und $g \in \Leb^1(\Omega, \Alg, \mu), g \geq 0$, sodass $\forall n \in \N : |f_n| \overset{\text{f.ü.}}\leq g$. Dann:\\[-16pt]
  \end{doublespace}
  \begin{align*}
...
\begin{satz}[von der majorisierten Konvergenz]\begin{doublespace}
  Sei $g \in \Leb^1(\Omega, \Alg, \mu), g \geq 0$. Sei $(f_n)_{n \in \N}$ Folge messbarer Fkt. $f_n : (\Omega, \Alg, \mu) \to (\ER, \overline{\Bor})$ mit $|f_n| \overset{\text{f.ü.}}\leq g$ (Majorisierung).
...
\begin{satz}\begin{doublespace}
  Seien $f, f_j : (\Omega, \Alg, \mu) \to (\R, \Bor), j \in \N$ messbare Funktionen, $g : (\Omega, \Alg, \mu) \to (\R, \Bor)$ integrierbar, sodass $|\sum_{j=1}^n f_j| \overset{\text{f.ü.}}\leq g \, \forall n \in \N$ und\\[-5pt]
...
\begin{satz}[Ableiten unter Integral]
  ...
  \begin{itemize}
    ...
    \item Es gibt eine Funktion $g \in \Leb^1(\Omega, \Alg, \mu)$ mit $g \geq 0$, sodass für alle $t \in ]a, b[$ und fast alle $\omega \in \Omega$ gilt: $|f(-, \omega)'(t)| \leq g(\omega)$.
  \end{itemize}
...
\begin{defn}
  ...
  \[ |Z| \coloneqq \max \Set{ x_j - x_{j-1} }{ j \in \{ 1, ..., n \} }. \]
\end{defn}
...
\begin{satz}
  Sei $f : [a, b] \to \R$ beschränkt und $(Z_k)_{k \in \N}$ eine Folge in $\mathcal{Z}([a, b])$ mit $\lim_{n \to \infty} |Z_k| = 0$. Dann gilt:
  \begin{itemize}
...
\begin{satz}
  ...
  \begin{itemize}
    \item $|f|$ ist auf $I$ uneigentlich Riemann-integrierbar.
...
\begin{satz}
  ...
  \[ A_* \mu_{LB} = \tfrac{1}{|\det(A)|} \cdot \mu_{LB}. \]
\end{satz}
